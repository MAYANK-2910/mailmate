...
import { StackListSkeleton } from './ui/Skeleton';
...
export function App() {
  ...
  if (isMinimized) {
    return (
      <div 
        className="fixed bottom-5 right-5 w-12 h-12 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-400 flex items-center justify-center cursor-pointer z-[999999] shadow-lg shadow-indigo-500/40 hover:scale-110 hover:shadow-xl hover:shadow-indigo-500/50 transition-all duration-200 text-xl pointer-events-auto"
  ...
  if (!isAuthenticated && !isLoading) {
    return (
      <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[999999] pointer-events-auto">
        <div className="bg-bg-primary w-[400px] h-[500px] rounded-2xl shadow-2xl flex flex-col relative overflow-hidden">
          <button 
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-[999999] pointer-events-auto flex justify-end">
      <div className="w-full max-w-2xl h-screen bg-bg-primary text-text-primary flex flex-col shadow-2xl border-l border-border animate-slide-in">
        <Header />
        <NavTabs />
